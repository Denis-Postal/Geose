...
function backButton() {
	$('body').append('<div id="pageFadeOut" style="position:fixed; inset:0; z-index:999999; background:#000; opacity:0; pointer-events:none; transition:opacity 180ms linear"></div>');
	requestAnimationFrame(() => $('#pageFadeOut').css('opacity', '1'));
	setTimeout(() => {
		const shouldReturnToCreator = sessionStorage.getItem("geoseReturnCreator") == "1";
		if (window.history.length > 1 && (shouldReturnToCreator || document.referrer.startsWith(window.location.origin))) {
			sessionStorage.removeItem("geoseReturnCreator");
			window.history.back();
		}
		else window.location.href = "/?openCreator=1";
	}, 190);
}

function launchGeoseLevel(id) {
	if (!id) return;
	sessionStorage.removeItem("geoseReturnCreator");
...
function Fetch(link) {
	return new Promise(function (res, rej) {
		if (link.startsWith("../api/")) link = "https://gdbrowser.com/api/" + link.slice("../api/".length)
