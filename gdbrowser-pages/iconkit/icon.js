...
function loadIconLayers(form, id, cb) {
    ...
    loader.add(texturesToLoad.filter(x => !loader.resources[x]).map(x => ({ name: x, url: `/gdbrowser-pages/iconkit/icons/${x}` })))
    loader.load(cb) // no params
}

// 2.2 icon spritesheets
function loadNewIcon(iconStr, cb) {
    fetch(`/gdbrowser-pages/iconkit/newicons/${iconStr}-hd.plist`).then(pl => pl.text()).then(plist => {

        let data = parseNewPlist(plist)
        let sheetName = iconStr + "-sheet"
        loader.add({ name: sheetName, url: `/gdbrowser-pages/iconkit/newicons/${iconStr}-hd.png` })
